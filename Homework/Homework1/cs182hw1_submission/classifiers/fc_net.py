...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx1, grads['W2'], grads['b2'] = affine_backward(dout, cache2)
        dx2, grads['W1'], grads['b1'] = affine_relu_backward(dx1, cache1)
        # L2 regularization
        # for W1
  
        loss = loss + 0.5 * self.reg * (self.params['W2']**2).sum()
        grads['W2'] += self.reg * self.params['W2']
        
        loss = loss + 0.5 * self.reg * (self.params['W1']**2).sum()
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        self.params['b1'] = np.zeros(hidden_dims[0], self.dtype)
        self.params['W2'] = np.random.normal(0, weight_scale, size=(hidden_dims[0], hidden_dims[1]))
        self.params['b2'] = np.zeros(hidden_dims[1], self.dtype)
        self.params['W3'] = np.random.normal(0, weight_scale, size=(hidden_dims[1],num_classes))
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        loss, dout = softmax_loss(scores, y)
        dx1, grads['W3'], grads['b3'] = affine_backward(dout, cache3)
        dx2, grads['W2'], grads['b2'] = affine_relu_backward(dx1, cache2)
        dx3, grads['W1'], grads['b1'] = affine_backward(dx2, cache1)

        loss += 0.5 * self.reg * ((self.params['W3'])**2).sum()
